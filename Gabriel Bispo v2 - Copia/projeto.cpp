...
void inicializa_tabuleiro(Part (&tabuleiro)[TAM][TAM]) {
    for (int i = 0; i < TAM; i++) {
        for (int j = 0; j < TAM; j++) { 
            tabuleiro[i][j].pos = {(float)inicioX + i * (diametro + espaco), // b + ax
                                   (float)inicioY + j * (diametro + espaco)}; // b + ax
            if ((i < 2 || i > 4) && (j < 2 || j > 4))
                tabuleiro[i][j].state = NOT_EXISTS; // Parte transparente, sem nada
            else
                tabuleiro[i][j].state = EXISTS;  // peças do jogo
        }
    }
    tabuleiro[3][3].state = EMPTY_SPACE; // posição do meio vazia
}
...
void desenha_tabuleiro(Part (&tabuleiro)[TAM][TAM], int i_atual, int j_atual, int clique_atual){
    ...
    for (int i = 0; i < TAM; i++) {
        for (int j = 0; j < TAM; j++) {
            ...
            if (tabuleiro[i][j].state == NOT_EXISTS) continue;

            Color cor = (tabuleiro[i][j].state == NONE) ? LIGHTGRAY : colo; 
