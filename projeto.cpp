...
bool valida_Part(Part(&tabuleiro)[TAM][TAM], int Io, int Jo, int (&S)){
    ...
    int Id, Jd, Im,Jm; /*-- Io,Jo origem, Id,Jd Destinho, Im,Jm Meio --*/
       ...
       // S retorna o valor por referencia
        if(tabuleiro[Io][Jo].state ==0) //parte que esta fora (em branco)
            return false;
        if(tabuleiro[Io][Jo].state !=2) // nao é uma peça valida
            return false;
            ...
            if (localize_Part(tabuleiro, &Ifim, &jfim, 1)) {
               ...
               int movimento = calcule_movimento(Ic, Jc, Ifim, jfim);
                ...
                if (movimento == MOVIMENTO_INVALIDO) {
                    DrawText("MOVIMENTO INVALIDO",100,65,30,RED);
                  //  Sleep(1000);
                } else {
                    ...
                    if (tabuleiro[i_alvo][j_alvo].state == 2) {
                        ...
                        tabuleiro[Ifim][jfim].state = 2;     //  atualiza o estado das peças movidas
                        tabuleiro[Ic][Jc].state = 1;         //
                        //-----------------------------------//
                    } 
                }
...
//----------------------------------------------------------------------------
// PROCEDIMENTO Capitura_Informacao ---> Para Verificar OS Cliques no MOUSE
//----------------------------------------------------------------------------
