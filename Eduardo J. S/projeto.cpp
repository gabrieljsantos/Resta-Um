...
bool trocarTema(void) {
    temaAtual = !temaAtual;  // ALTERNAR TRUE/FALSE
      ...
      if(temaAtual){
        ...
        corTabu = BROWN;
        corTime = BLACK;
        std::cout << "Tema 1: Branco + Marrom" << std::endl;
        }
        else{
        // tama 2: escuro
        corFundo = BLACK;
...
void desenha_tabuleiro(Part (&tabuleiro)[TAM][TAM], int i_atual, int j_atual, bool red){
    DrawRectangle(inicioX-50, inicioY-50, 460, 460, corPeca);//| QUADRADO FUNDO
...
if(locale_Muda()){
    ...
    if(!jogada_Valida(tabuleiro, R)){
        if (R == 1) {
            ...
            DrawText("GAME OVER", 900/2 -100, 600/2, 40, RED);
            DrawText(TextFormat("Restaram: %d", R), 100, 150, 30, corBotao);
        }
    }
}
...
void DisplayTimer(void) {
    ...
    DrawText(textoTimer, 650, 20, 20, corTime);
}

//----------------------------------------------------------------------------
// PROCEDIMENTO Atualiza_Imformação ---> Para atualizar as vareaveis dos clik
//----------------------------------------------------------------------------
